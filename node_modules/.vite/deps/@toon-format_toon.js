...
function* decodeStreamSync$1(source, options) {
  ...
  const resolvedOptions = {
    indent: (options == null ? void 0 : options.indent) ?? 2,
    strict: (options == null ? void 0 : options.strict) ?? true
  };
...
function expandPathsSafe(value, strict) {
  ...
  if (isJsonObject(value)) {
    ...
    for (const [key, keyValue] of Object.entries(value)) {
      const isQuoted = quotedKeys == null ? void 0 : quotedKeys.has(key);
...
function formatHeader(length, options) {
  const key = options == null ? void 0 : options.key;
  const fields = options == null ? void 0 : options.fields;
  const delimiter = (options == null ? void 0 : options.delimiter) ?? COMMA;
  let header = "";
...
function resolveOptions(options) {
  return {
    indent: (options == null ? void 0 : options.indent) ?? 2,
    delimiter: (options == null ? void 0 : options.delimiter) ?? DEFAULT_DELIMITER,
    keyFolding: (options == null ? void 0 : options.keyFolding) ?? "off",
    flattenDepth: (options == null ? void 0 : options.flattenDepth) ?? Number.POSITIVE_INFINITY,
    replacer: options == null ? void 0 : options.replacer
  };
}
function resolveDecodeOptions(options) {
  return {
    indent: (options == null ? void 0 : options.indent) ?? 2,
    strict: (options == null ? void 0 : options.strict) ?? true,
    expandPaths: (options == null ? void 0 : options.expandPaths) ?? "off"
  };
}
export {
  DEFAULT_DELIMITER,
...
//# sourceMappingURL=@toon-format_toon.js.map
